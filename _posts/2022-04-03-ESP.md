...
_This was volunteer open-source work with the [Earth Species Project](https://www.earthspecies.org/) in late 2020. The dataset was a curated collection of Egyptian fruit bat vocalizations created by [Yossi Yovel](http://www.yossiyovel.com/index.php/publications) at Tel Aviv University. The task: predict the behavioral context of a vocalization (feeding, fighting, mating protest, etc.) and the gender of the bat. The benchmark from Yovel's 2017 Nature paper was 61% on the context classification task. The architecture below pushes that to 67%._
...
annotations["Addressee sex"] = [addressee_sex_dict[x] 
                                for x in annotations["Addressee"]]
...
```python
annotations["Addressee sex"].value_counts().plot(kind = 'bar', 
...
def pos_enc2d_np(gs_im, out_c = 5, eta = 0.05):
    ...
    def make_mask(shape, start, end):
        ...
        block_2 = np.zeros((shape[1], 
                            constrain(shape[0] - end, 
                            0, shape[0])))
